...
class BooksViewModel(// принимает в себя все кейсы
    ...
    fun getBooks(author: String) {// для асинхронной работы
    ...
    fun unbookmark(book: BookWithStatus) {
        viewModelScope.launch {
            unbookmarkBookUseCase.invoke(mapper.fromBookWithStatusToVolume(book))// результат работы маппера
        }
    }

    class BooksViewModelFactory(// чтобы создавать наш вью модел
        private val getBooksUseCase: GetBooksUseCase,
        private val getBookmarksUseCase: GetBookmarksUseCase,
        private val bookmarkBookUseCase: BookmarkBookUseCase,
        private val unbookmarkBookUseCase: UnbookmarkBookUseCase,
